...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  // Gate persistence until we've had a chance to restore from the on-disk file.
  // On Tauri the webview localStorage can come up empty after a reboot; the file
  // is the source of truth. Without this gate, the save effect below fires on
  // mount and writes DEFAULT settings over localStorage before the async file
  // read runs, so the file read then bails and every setting resets on boot.
  ...
  useEffect(() => {
    let cancelled = false;
    void (async () => {
      // If localStorage already has settings (web, or a webview that kept them),
      // trust those. Otherwise restore from the on-disk file (Tauri reboot case).
      if (!localStorage.getItem(STORAGE_KEY)) {
        const raw = await readSettingsFile();
        if (!cancelled && raw && !localStorage.getItem(STORAGE_KEY)) {
          try {
  ...
  useEffect(() => {
    // Don't persist until the on-disk file has had its chance to load, or the
    // initial DEFAULT state would clobber the saved settings on every boot.
    if (!loaded) return;
    try {
