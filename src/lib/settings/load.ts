...
export function loadStoredSettings(): Settings {
  ...
  try {
    const parsed = JSON.parse(raw) as Partial<Settings> & {
      ...
      _customListsFavSeedV1?: boolean;
      _rtBadgesOnV1?: boolean;
      scrapers?: unknown;
    ...
    // Seed the MDBList "Favorites" list for existing users (their persisted
    // customLists override the default seed in the spread below).
    if (!parsed._customListsFavSeedV1) {
      const existing = Array.isArray(parsed.customLists) ? parsed.customLists : [];
      if (!existing.some((l) => l.id === "cl-favorites-rated")) {
        const seed = DEFAULT.customLists.find((l) => l.id === "cl-favorites-rated");
        if (seed) parsed.customLists = [...existing, seed];
      }
      parsed._customListsFavSeedV1 = true;
    }
    // Rotten Tomatoes critics + audience badges should be on everywhere by
    // default. Flip them on once for existing users who had the old off default.
    if (!parsed._rtBadgesOnV1) {
      parsed.showRtBadge = true;
      parsed.showPopcornBadge = true;
      parsed.showRtBanner = true;
      parsed._rtBadgesOnV1 = true;
    }
    ...
    return {
      ...
      // RT badge was one shared flag; banner is now independent. Existing users
      // who had RT on keep it in the banner; otherwise fall back to the default.
      showRtBanner: parsed.showRtBanner ?? parsed.showRtBadge ?? DEFAULT.showRtBanner,
      badgePlacement:
