...
export type LibraryItem = {
  ...
  _ctime: string;
  _mtime: string;
  external?: "simkl" | "trakt";
  isAnime?: boolean;
  upNext?: boolean;
  local?: boolean;
  /** Next-episode title for Up Next items, so the card can label it without a lookup. */
  episodeTitle?: string;
  /** Series title for Up Next items (item.name is kept empty for these). */
  seriesTitle?: string;
};
...
export function isCwMember(i: LibraryItem): boolean {
  ...
  // Trakt up-next entries are the next episode to watch (not yet started), so
  // they have no progress; include them in Continue Watching regardless.
  if (i.upNext) return true;
  if (!i.state) {
