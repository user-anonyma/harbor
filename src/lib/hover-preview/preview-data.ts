...
import { mdblistCardCached } from "../providers/mdblist-batch";
...
function deriveRating(meta: Meta, isAnime: boolean): PreviewData["rating"] {
  ...
  const kind = meta.type === "series" ? "show" : "movie";
  const mdbImdb = mdblistCardCached(imdbId, kind)?.imdb;
  const value =
    (imdbId ? omdbScoresCached(imdbId)?.imdbRating : undefined) ??
    meta.imdbRating ??
    (mdbImdb != null ? mdbImdb.toFixed(1) : undefined);
  return value ? { kind: "imdb", value } : null;
}

function deriveRt(meta: Meta, isAnime: boolean): number | null {
  if (isAnime) return null;
  const imdbId = meta.id.startsWith("tt") ? meta.id : tmdbImdbCached(meta.id) ?? undefined;
  const kind = meta.type === "series" ? "show" : "movie";
  return (
    (imdbId ? omdbScoresCached(imdbId)?.rtCritics : undefined) ??
    mdblistCardCached(imdbId, kind)?.rtCritics ??
    null
  );
}

function deriveRtAudience(meta: Meta, isAnime: boolean): number | null {
  if (isAnime) return null;
  const imdbId = meta.id.startsWith("tt") ? meta.id : tmdbImdbCached(meta.id) ?? undefined;
  const kind = meta.type === "series" ? "show" : "movie";
  return mdblistCardCached(imdbId, kind)?.rtAudience ?? null;
}
...
export function assemblePreviewData(meta: Meta): PreviewAssembly {
  ...
  const rtCritics = deriveRt(meta, isAnime);
  const rtAudience = deriveRtAudience(meta, isAnime);
