...
function parse(json: ApiShape): MdblistScores {
  ...
  return {
    ...
    rtCritics: val("tomatoescritics") ?? val("tomatoes") ?? val("critics"),
  };
}

// Does this result carry any actual rating? Used to decide whether to retry the
// other media type (mdblist returns an empty/absent payload for a wrong type).
function hasAnyRating(s: MdblistScores | null): boolean {
  if (!s) return false;
  return (
    s.imdb != null ||
    s.tmdb != null ||
...
async function fetchScores(
  ...
  const primary = await fetchOne(key, imdbId, type);
  if (hasAnyRating(primary)) return primary;
  // Wrong movie/show label yields an empty payload; retry the other type so a
  // mislabeled title still gets its full ratings.
  const alt = await fetchOne(key, imdbId, type === "movie" ? "show" : "movie");
  return hasAnyRating(alt) ? alt : primary;
}
