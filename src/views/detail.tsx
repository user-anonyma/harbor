...
export function DetailView({
  ...
  const [watchedMark, setWatchedMark] = useState(false);
  ...
  // Arctic Fuse detail zoom-out: as you scroll/focus down into the content below
  // the hero, the header recedes (dims + shrinks slightly) instead of just
  // scrolling away, so attention moves cleanly to the widgets. Scroll-driven so
  // it works identically for the remote (focusing a card scrolls it into view)
  // and the mouse. Skipped under prefers-reduced-motion.
  useEffect(() => {
    ...
    const apply = () => {
      raf = 0;
      const h = hero.offsetHeight || 640;
      const p = Math.min(1, Math.max(0, scroller.scrollTop / (h * 0.55)));
      hero.style.transform = `scale(${(1 - p * 0.06).toFixed(4)})`;
      hero.style.opacity = (1 - p * 0.7).toFixed(3);
    };
    const onScroll = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      if (k != null && k !== failedKitsu.current) setDetectedKitsu(k);
      setDetectingAnime(false);
    })().catch(() => {
    ...
    >
      <section className="relative">
        <div
          ...
          className="harbor-bleed-stremio relative h-[78vh] min-h-[640px] overflow-hidden transition-[transform,opacity] duration-200 ease-out"
        >
              ...
              <div ref={actionRowRef} className="mt-9 flex items-center gap-3 [&>*]:shrink-0">
                {upcoming ? (
                  ...
                  <button
                    ...
                    className="harbor-card-focus flex h-12 items-center gap-2.5 rounded-full bg-ink px-7 text-[15px] font-semibold text-canvas shadow-[0_8px_24px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.65),inset_0_-1px_0_rgba(0,0,0,0.18)] transition-transform duration-200 hover:scale-[1.03] active:scale-[0.98]"
                  >
                  ...
                  <button
                    ...
                    onClick={() =>
                      toggleWatchlist({
                        ...
                        poster: overridePoster ?? meta.poster ?? detail?.poster,
