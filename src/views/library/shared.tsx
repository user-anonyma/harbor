...
export type Tab = "watchlist" | "history" | "favorites" | "anilist" | "simkl";
...
export async function hydrateLibraryMeta(
  ...
  const p = (async () => {
    // TMDB-first: resolve to a tmdb id (tmdb: passes through, imdb resolves via
    // /find) and pull poster + backdrop from TMDB so the library matches the
    // rest of the app. Falls back to addon/cinemeta meta if TMDB has nothing.
    if (tmdbKey) {
      let tmdbId: string | null = null;
      if (id.startsWith("tmdb:")) {
        tmdbId = id.replace(/^tmdb:series:/, "tmdb:tv:");
      } else if (id.startsWith("tt")) {
        tmdbId = await tmdbIdFromImdb(tmdbKey, id, type).catch(() => null);
      }
      if (tmdbId) {
        const lite = await tmdbLiteMeta(tmdbKey, tmdbId).catch(() => null);
        if (lite?.poster) {
          return {
            id,
            type,
            name: lite.name ?? "",
            poster: lite.poster ?? undefined,
            background: lite.background ?? undefined,
          } as Meta;
        }
      }
    }
...
export function WatchlistCard({ meta, onRemove }: { meta: Meta; onRemove?: () => void }) {
  ...
  const watch = useWatchState(meta.id);
  ...
  return (
    ...
    >
      <div
        ...
        tabIndex={0}
        onClick={open}
        onContextMenu={(e) => openContextMenu(e, { kind: "meta", meta: display })}
        onKeyDown={(e) => {
      ...
      >
        <Poster
          src={overridePoster || display.poster}
          seed={display.id}
          className="h-full w-full"
          onError={onPosterError}
        />
        {watch?.watched ? (
          <span className="pointer-events-none absolute bottom-1.5 end-6 z-10 flex h-[22px] w-[22px] items-center justify-center rounded-full bg-black ring-2 ring-[#1a8f42] shadow-[0_0_8px_rgba(26,143,66,0.55)]">
