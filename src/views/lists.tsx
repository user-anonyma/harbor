...
export default function ListsView({ active }: { active: boolean }) {
  ...
  // Split a mixed list into Films / Series for the Kodi-style tabbed wall.
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-6 pt-6 pb-16">
          {!activeList ? (
            ...
            <>
              ...
              {hasBoth && (
                <div className="mb-6 flex items-center gap-6 border-b border-edge-soft/30">
                  {(["films", "series"] as const).map((k) => (
                    <button
                      key={k}
                      type="button"
                      onClick={() => setTab(k)}
                      className={`relative -mb-px pb-2.5 text-[15px] font-medium transition-colors ${
                        tab === k ? "text-ink" : "text-ink-subtle hover:text-ink-muted"
                      }`}
                    >
                      {k === "films" ? t("Films") : t("Series")}
                      {tab === k && (
              ...
              {shown.length > RENDER_CAP && (
                <p className="mb-4 text-[12.5px] text-ink-subtle">
                  {t("Showing {shown} of {total}.", { shown: RENDER_CAP.toLocaleString(), total: shown.length.toLocaleString() })}
                </p>
              )}
