...
import { fetchWatchedKeySet, fetchWatchedTitleKeys } from "@/lib/trakt/history";
...
import { fetchTraktUpNext } from "@/lib/trakt/up-next";
...
export function Home({ active = true }: { active?: boolean }) {
  ...
  const [traktCw, setTraktCw] = useState<LibraryItem[]>([]);
  ...
  useEffect(() => {
    ...
    fetchWatchedKeySet()
      ...
      .catch(() => {});
    // Full watched library (not capped history) for the app-wide watched icon.
    fetchWatchedTitleKeys()
      .then((set) => {
        if (!cancelled) setTraktWatchedIndex(set);
      })
      .catch(() => {});
    // Trakt up-next (next unwatched episode per show), feeds Continue Watching.
    fetchTraktUpNext()
      .then((cw) => {
        if (!cancelled) setTraktCw(cw);
      })
      .catch(() => {});
    return () => {
      cancelled = true;
  ...
  const continueWatching = useMemo(() => {
    ...
    // Continue Watching is the user's Trakt "Up Next" list and nothing else.
    // When Trakt is connected we show ONLY those items (no Stremio in-progress,
    // no Simkl, no local guesses). Only if there's no Trakt Up Next at all do we
    // fall back to the merged sources so non-Trakt users still get a row.
    const source = traktCw.length > 0 ? traktCw : [...items, ...simklCw, ...localCwItems];
    ...
    return out;
  }, [items, simklCw, traktCw, localCwVer, cwVersion, settings.animeOnlyInAnimeRoom, animeDetectVer]);
  ...
  return (
    <main
      ref={scrollCb}
      className="flex-1 overflow-y-auto px-5 pt-8 pb-14 sm:px-8 lg:px-12 lg:pt-10"
    >
      <ScrollRootContext.Provider value={scrollEl}>
        <div data-tauri-drag-region className="relative flex flex-col gap-8">
