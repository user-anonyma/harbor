...
import { PopcornBadge as PopcornMeter } from "@/components/popcorn-badge";
      ...
      >
        ...
        <div className="mt-2 border-t border-edge-soft/60 pt-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            />
            <ToggleRow
              label={t("Show Rotten Tomatoes score in the banner")}
              sub={t("Shows the RT critics score on the title's hero banner instead of the poster cards.")}
              leading={<RtPairBadge />}
              value={settings.showRtBanner}
              onChange={(v) => update({ showRtBanner: v })}
              lockReason={
                !settings.omdbKey
                  ? t("Add an OMDb key above to unlock this.")
                  : undefined
...
function PopcornBadge() {
  return (
    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-canvas ring-1 ring-edge-soft">
      <PopcornMeter score={100} className="h-[15px] w-auto" />
    </span>
  );
}
  ...
  if (f.showPopcorn)
    out.push(
      <span className="flex items-center gap-0.5">
        <PopcornMeter score={85} className="h-[12px] w-auto" />
        <span>85%</span>
      </span>,
    );
  if (f.showMetacritic)
