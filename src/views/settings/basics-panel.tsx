  ...
  return (
    <>
      <Section
        ...
        subtitle={t("Used across the whole app for highlights, focus, and buttons.")}
      >
        <div className="px-1 py-1">
          <ColorPicker
            value={settings.accentColor || "#e5a13a"}
      ...
      <Section
        title={t("Sidebar")}
        subtitle={t("How the side navigation behaves. On a TV, Always expanded or Icons only are clearest.")}
      >
        <div className="flex flex-wrap gap-2 px-1 py-1">
          {(
            [
              ["expanded", t("Always expanded")],
              ["collapsed", t("Icons only")],
              ["auto", t("Auto collapse")],
              ["remember", t("Remember last")],
            ] as const
          ).map(([k, label]) => (
            <button
              key={k}
              type="button"
              onClick={() => update({ sidebarBehavior: k })}
              className={`harbor-card-focus rounded-xl px-4 py-2.5 text-[14px] font-medium outline-none transition-colors ${
                settings.sidebarBehavior === k
                  ? "bg-accent text-white"
                  : "bg-elevated/50 text-ink-muted ring-1 ring-edge-soft/50 hover:text-ink"
              }`}
            >
              {label}
            </button>
