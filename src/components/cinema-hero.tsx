...
import { HeroRatingBadges } from "@/components/hero-rating-badges";
...
export function CinemaHero({
  ...
  if (slides.length === 0) {
    return (
      <section className="harbor-bleed-stremio relative h-[440px] animate-pulse bg-elevated/30" />
    );
  }
  ...
  return (
    <section
      ref={ref}
      className="harbor-bleed-stremio relative h-[440px] overflow-hidden bg-canvas"
      onMouseEnter={() => setPaused(true)}
      onMouseLeave={() => setPaused(false)}
      onKeyDown={(e) => {
        // Left/Right move between banners (like the dots) so the remote isn't
        // stuck on the hero; Up/Down fall through to normal navigation.
        if (slides.length < 2) return;
        if (e.key === "ArrowLeft") {
          e.preventDefault();
          e.stopPropagation();
        ...
        <div className="flex max-w-[640px] flex-col gap-5">
          ...
          <div className="flex flex-wrap items-center gap-x-5 gap-y-1.5 text-[13px] tracking-wide text-ink-muted">
            ...
            <HeroRatingBadges
              ...
              imdbId={resolvedImdb}
              kind={meta.type === "series" ? "series" : "movie"}
