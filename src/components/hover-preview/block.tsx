...
function DecisionLine({ data }: { data: PreviewData }) {
  ...
  if (data.rating && (data.rating.kind === "mal" || settings.showImdbBadge)) {
    parts.push(
  ...
  if (data.rtCritics != null && settings.showRtBadge) {
    parts.push(
      <span key="rt" className="inline-flex items-center gap-1 align-middle">
        <RtBadge score={data.rtCritics} className="h-[12px] w-auto" />
        <span className="font-semibold text-ink">{data.rtCritics}%</span>
      </span>,
    );
  }
  if (data.rtAudience != null && settings.showPopcornBadge) {
    parts.push(
      <span key="rt-aud" className="inline-flex items-center gap-1 align-middle">
        <PopcornBadge score={data.rtAudience} className="h-[12px] w-auto" />
        <span className="font-semibold text-ink">{Math.round(data.rtAudience)}%</span>
      </span>,
    );
  }
