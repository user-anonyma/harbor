...
export function Row({
  ...
  const measure = () => {
    ...
    // When there are more items than fit, leave a sliver of the next poster
    // visible on the trailing edge (Kodi / Netflix style) so it's obvious the
    // row keeps scrolling. If everything fits, fill the row edge to edge.
    const PEEK = 30;
    const hasMore = childCount > fits;
    const usable = hasMore ? Math.max(effMin, available - PEEK) : available;
    const fitsU = Math.max(1, Math.floor((usable + GAP) / (effMin + GAP)));
    setCellWidth((usable - (fitsU - 1) * GAP) / fitsU);
  };
      ...
      {title && (
        <div className="flex items-baseline justify-between gap-4 pe-1">
          <h3
            ...
            style={{ fontSize: `${Math.round(20 * settings.rowTitleScale)}px` }}
          >
            {title}
          </h3>
        </div>
      ...
      <div ref={containerRef} className="group/row relative min-w-0">
        <RowTrackContext.Provider value={trackEl}>
          ...
          >
            ...
            {onViewAll && (canPrev || canNext) && (
              <ViewAllCard shape={shape} label={t(viewAllLabel)} onClick={onViewAll} />
            )}
          </div>
...
// End-of-row card (Harbor-styled) that stands in for the old "View all" text.
// Only rendered when the row overflows (more items than fit).
function ViewAllCard({
  ...
  const aspect = shape === "landscape" ? "aspect-video" : "aspect-[2/3]";
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      <div
        className={`harbor-card-ring ${aspect} flex w-full items-center justify-center rounded-xl border border-edge-soft bg-elevated/40 text-ink-muted transition-colors duration-200 group-hover:text-ink group-focus-within:text-ink group-hover:bg-elevated/70 group-focus-within:bg-elevated/70`}
      >
