...
export function BigCardStack({
  ...
  const mdblist = useMdblistCardScores(
    resolvedImdb ?? undefined,
    current.type === "series" ? "show" : "movie",
  );
  ...
  return (
    <div
      ...
      className={`harbor-card-focus group relative block h-full min-h-[420px] w-full min-w-0 overflow-hidden rounded-2xl border border-edge-soft bg-canvas text-start transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0.24,1)] hover:-translate-y-1 ${
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2.5 text-[13px] text-ink/80">
          ...
          {settings.showRtBanner && mdblist?.rtCritics != null && (
            <>
              <Dot />
              <span className="inline-flex items-center gap-1.5">
                <RtBadge score={mdblist.rtCritics} className="h-[14px] w-auto" />
                {mdblist.rtCritics}%
              </span>
            </>
          )}
          {settings.showRtBanner && mdblist?.rtAudience != null && (
            <>
              <Dot />
              <span className="inline-flex items-center gap-1.5">
                <PopcornBadge score={mdblist.rtAudience} className="h-[14px] w-auto" />
                {Math.round(mdblist.rtAudience)}%
              </span>
            </>
          )}
        </div>
