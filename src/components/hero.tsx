...
export const Hero = memo(function Hero({
  ...
  const heroMdblist = useMdblistCardScores(
  ...
  return (
    <section
      ...
      className="group relative h-[440px] cursor-pointer overflow-hidden rounded-[28px] bg-canvas"
      ...
      <div className="relative flex h-full flex-col justify-center p-14">
        <div className="max-w-2xl">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-x-8 gap-y-2 text-[14px]">
            ...
            {settings.showRtBanner && (omdb?.rtCritics ?? heroMdblist?.rtCritics) != null && (
              <span className="flex items-center gap-2">
                <RtBadge score={(omdb?.rtCritics ?? heroMdblist?.rtCritics)!} className="h-[18px] w-auto" />
                <span className="font-semibold text-ink">
                  {omdb?.rtCritics ?? heroMdblist?.rtCritics}%
                </span>
              </span>
            )}
            {settings.showRtBanner && heroMdblist?.rtAudience != null && (
              <span className="flex items-center gap-2">
                <PopcornBadge score={heroMdblist.rtAudience} className="h-[18px] w-auto" />
                <span className="font-semibold text-ink">{Math.round(heroMdblist.rtAudience)}%</span>
              </span>
            )}
