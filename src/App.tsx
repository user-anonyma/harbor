...
import { ArtworkPickerHost } from "@/components/artwork-picker";
...
import { initSpatialNav, setSpatialNavSuspended, rememberFocus, restoreOrDefaultFocus } from "@/lib/spatial-nav";
...
import { AuthProvider, readActiveStremioAuthKey } from "@/lib/auth";
import { library } from "@/lib/stremio";
import { fetchWatchedTitleKeys } from "@/lib/trakt/history";
import { setWatchStateIndex, setTraktWatchedIndex } from "@/lib/watch-state";
...
import { TraktProvider, useTrakt } from "@/lib/trakt/provider";
...
// Remembers where focus was on each page and restores it when you return
// (Arctic focus memory), else lands on the page's default control. Remote-only.
function FocusMemory() {
  const { view, topKind, meta } = useView();
  const routeKey = topKind === "meta" && meta ? `meta:${meta.id}` : String(view ?? topKind ?? "home");
  const prevRef = useRef(routeKey);
  useEffect(() => {
    if (prevRef.current === routeKey) return;
    rememberFocus(prevRef.current);
    prevRef.current = routeKey;
    const id = window.setTimeout(() => restoreOrDefaultFocus(routeKey), 160);
...
function Shell() {
  ...
  // D-pad / arrow-key navigation across the whole app (TV / remote support).
  useEffect(() => initSpatialNav(), []);

  // Custom accent color, applied app-wide via the --color-accent CSS variable.
  useEffect(() => {
    const root = document.documentElement;
    const c = settings.accentColor?.trim();
    if (c) {
      root.style.setProperty("--color-accent", c);
      root.style.setProperty("--color-accent-soft", `color-mix(in srgb, ${c} 18%, transparent)`);
    } else {
  ...
  // Load watched-state app-wide (not only on the home view) so the green check
  // / in-progress ring is correct on every page, including the Library.
  const { isConnected: traktConnectedForWatch } = useTrakt();
  useEffect(() => {
    let cancelled = false;
    ...
    if (authKey) {
      library(authKey)
        .then((items) => {
          if (!cancelled) setWatchStateIndex(items);
        })
        .catch(() => {});
    }
    if (traktConnectedForWatch) {
      fetchWatchedTitleKeys()
        .then((s) => {
          if (!cancelled) setTraktWatchedIndex(s);
        })
        .catch(() => {});
    }
    return () => {
  ...
  }, [traktConnectedForWatch]);
  // The player owns arrow keys (seek / volume), so suspend spatial nav there.
  useEffect(() => {
    setSpatialNavSuspended(!!player);
  }, [player]);
